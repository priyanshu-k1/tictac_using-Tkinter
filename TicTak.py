...
global tiles1,tiles2,tiles3,tiles4,tiles5,tiles6,tiles7,tiles8,tiles9,MakeAiMove,checkISwinning,AiMode,PreventRepetation,listcounter
global counter
counterxyz=0
AiMode=0
PreventRepetation=[1,2,3,4,5,6,7,8,9]
random.shuffle(PreventRepetation)
...
def ScoreCounterUpdater():
    ...
    print(f"Score of player 1{PlayerOneScoreMatrix}")
    print(f"Score of player 2{PlayerTwoScoreMatrix}")
		...
		if(AiMode==1):
			if not (check_zero(matrix) or check_one(matrix)):
				if(GlobalCounterForXandO==1):
					...
					sleep(0.6)
					FirstPlayerLabel.config(text="Your chance sir...")
					checkISwinning()
			else:
				pass
		else:
...
def MakeAiMove(num):
    if(num==1):
        ...
        pass
    print(PreventRepetation)
# have to improve ai moving logic ::::chala imortant
def checkISwinning():
  global listcounter
  print(f"{matrix}\n")
  #1st row
  if((matrix[0][0]==1 and matrix[0][1]==1 ) or (matrix[0][0]==0  and matrix[0][1]==0)):
      print("(0,2)")
      try:
      	MakeAiMove(3)
      except:
      	MakeAiMove(random.choice(PreventRepetation))
  elif((matrix[0][2]==1 and matrix[0][1]==1 ) or ( matrix[0][2]==0 and matrix[0][1]==0)):
    print("(0,0)")
    MakeAiMove(1)
  elif((matrix[0][0]==1 and matrix[0][2]==1) or (matrix[0][0]==0  and matrix[0][2]==0)):
    print("(0,1)")
    try:
    	MakeAiMove(2)
    except:
      	MakeAiMove(random.choice(PreventRepetation))
  #2nd row
  elif((matrix[1][0]==1 and matrix[1][1]==1 ) or ( matrix[1][1]==0 and matrix[1][0]==0 )):
      print("(1,2)")
      try:
      	MakeAiMove(6)
      except:
      	MakeAiMove(random.choice(PreventRepetation))
  elif((matrix[1][2]==1 and matrix[1][1]==1 ) or (matrix[1][2]==0 and matrix[1][1]==0)):
    print("(1,0)")
    try:
    	MakeAiMove(3)
    except:
      	MakeAiMove(random.choice(PreventRepetation))
  elif((matrix[1][0]==1 and matrix[1][2]==1) or ( matrix[1][0]==0 and matrix[1][2]==0)):
    print("(1,1)")
    try:
    	MakeAiMove(5)
    except:
      	MakeAiMove(random.choice(PreventRepetation))
  #3rd  row
  elif((matrix[2][0]==1 and matrix[2][1]==1 ) or (matrix[2][0]==0  and matrix[2][1]==0)):
      print("(2,2)")
      try:
      	MakeAiMove(9)
      except:
      	MakeAiMove(random.choice(PreventRepetation))
  elif((matrix[2][2]==1 and matrix[2][1]==1 ) or ( matrix[2][2]==0  and matrix[2][1]==0)):
    print("(2,0)")
    try:
    	MakeAiMove(7)
    except:
      	MakeAiMove(random.choice(PreventRepetation))
  elif((matrix[2][0]==1 and  matrix[2][2]==1 ) or (matrix[2][0]==0 and matrix[2][2]==0)):
    print("(2,1)")
    try:
    	MakeAiMove(8)
    except:
      	MakeAiMove(random.choice(PreventRepetation))
  #colomn one
  elif((matrix[0][0]==1 and matrix[1][0]==1) or (matrix[0][0]==0  and matrix[1][0]==0)):
      print("(2,0)")
      try:
      	MakeAiMove(7)
      except:
      	MakeAiMove(random.choice(PreventRepetation))
  elif((matrix[2][0]==1 and matrix[1][0]==1 ) or (matrix[2][0]==0  and matrix[1][0]==0)):
      print("(0,0)")
      try:
      	MakeAiMove(1)
      except:
      	MakeAiMove(random.choice(PreventRepetation))
  elif((matrix[2][0]==1 and matrix[0][0]==1) or ( matrix[2][0]==0  and matrix[0][0]==0)):
      print("(1,0)")
      try:
      	MakeAiMove(4)
      except:
      	MakeAiMove(random.choice(PreventRepetation))
  #colomn two
  elif((matrix[0][1]==1 and matrix[1][1]==1) or (matrix[0][1]==0  and matrix[1][1]==0)):
      print("(2,1)")
      try:
      	MakeAiMove(8)
      except:
      	MakeAiMove(random.choice(PreventRepetation))
  elif((matrix[2][1]==1 and matrix[1][1]==1 ) or (matrix[2][1]==0  and matrix[1][1]==0)):
      print("(0,1)")
      try:
      	MakeAiMove(2)
      except:
      	MakeAiMove(random.choice(PreventRepetation))   
  elif((matrix[2][1]==1 and matrix[0][1]==1) or (matrix[2][1]==0  and matrix[0][1]==0)):
      print("(1,1)")
      try:
      	MakeAiMove(5)
      except:
      	MakeAiMove(random.choice(PreventRepetation))
  #colomn three
  elif((matrix[0][2]==1 and matrix[1][2]==1) or (matrix[0][2]==0  and matrix[1][2]==0)):
      print("(2,2)")
      try:
      	MakeAiMove(9)
      except:
      	MakeAiMove(random.choice(PreventRepetation)) 
  elif((matrix[2][2]==1 and matrix[1][2]==1) or (matrix[2][2]==0  and matrix[1][2]==0)):
      print("(0,2)")
      try:
      	MakeAiMove(3)
      except:
      	MakeAiMove(random.choice(PreventRepetation))
  elif((matrix[2][2]==1 and matrix[0][2]==1) or ( matrix[2][2]==0  and matrix[0][2]==0)):
      print("(1,2)")
      try:
      	MakeAiMove(6)
      except:
      	MakeAiMove(random.choice(PreventRepetation))
  #Diagnoal
  elif((matrix[0][0]==1 and matrix[1][1]==1) or (matrix[0][0]==0  and matrix[1][1]==0)):
    print("(2,2)")
    try:
    	MakeAiMove(9)
    except:
      	MakeAiMove(random.choice(PreventRepetation))
  elif((matrix[0][0]==1 and matrix[2][2]==1) or (matrix[0][0]==0  and matrix[2][2]==0)):
    print("(1,1)")
    try:
   			MakeAiMove(5)
    except:
      	MakeAiMove(random.choice(PreventRepetation))
  elif((matrix[1][1]==1 and matrix[2][2]==1) or (matrix[1][1]==0  and matrix[2][2]==0)):
    print("(0,0)")
    try:
    	MakeAiMove(1) 
    except:
      	MakeAiMove(random.choice(PreventRepetation))
  elif((matrix[0][2]==1 and matrix[1][1]==1) or (matrix[0][2]==0  and matrix[1][1]==0)):
    print("(2,1)")
    try:
    	MakeAiMove(7)
    except:
      	MakeAiMove(random.choice(PreventRepetation)) 
  elif((matrix[0][2]==1 and matrix[2][1]==1) or (matrix[0][2]==0  and matrix[2][1]==0)):
    print("(1,1)")
    try:
    	MakeAiMove(5)
    except:
...
AgainstAi=Button(win,text="Against A.I",width=15,command=AgainstAiFunc,bg='light green')
